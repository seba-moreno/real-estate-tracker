...
def list_contracts(db: Session = Depends(get_db)) -> Optional[list[ContractResponse]]:
    service = ContractService(db)
...
def create_contract(
    contract: CreateContract, db: Session = Depends(get_db)
) -> ContractResponse:
    service = ContractService(db)
...
def update_contract(
    contract_id: int, contract: UpdateContract, db: Session = Depends(get_db)
) -> ContractResponse:
    service = ContractService(db)
