...
def get_concept(
    concept_id: int, db: Session = Depends(get_db)
...
def list_concepts(db: Session = Depends(get_db)) -> list[ConceptResponse]:
    service = ConceptService(db)
...
def create_concept(
    concept: CreateConcept, db: Session = Depends(get_db)
) -> ConceptResponse:
    service = ConceptService(db)
...
def update_concept(
    concept_id: int, concept: UpdateConcept, db: Session = Depends(get_db)
) -> ConceptResponse:
    service = ConceptService(db)
