...
def list_propertys(db: Session = Depends(get_db)) -> list[PropertyResponse]:
    service = PropertyService(db)
...
def create_property(
    property: CreateProperty, db: Session = Depends(get_db)
) -> PropertyResponse:
    service = PropertyService(db)
...
def update_property(
    property_id: int, property: UpdateProperty, db: Session = Depends(get_db)
) -> PropertyResponse:
    service = PropertyService(db)
