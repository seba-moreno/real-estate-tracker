...
class TransactionBase(BaseModel):
    ...
    properties_concepts_id: Annotated[int, Field(ge=1, description="Id of the properties_concepts that the Transaction belongs to")]
    transaction_type: Annotated[str, Field(description="Type of transaction in lowercase ('income' or 'expense')")]
    period: Annotated[str, Field(description="Year and month that the Transaction due date belongs to (2026-01)")]
    amount: Annotated[Decimal, Field(max_digits=19, decimal_places=2, ge=0, description="Amount of the Transaction (max 19 digits, 2 decimal places))")]
    ...
    def validate_amount(cls, v: Decimal) -> Decimal:
        if v < 0:
            raise ValueError("amount cannot be negative because transaction_type determines the sign.")
