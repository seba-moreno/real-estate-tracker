...
class ContractService:
    ...
    def get_contract_by_id(self, contract_id: int) -> Optional[ContractResponse]:
        ...
        if not existing_contract:
            self.logger.warning("Get by id failed: Contract not found", extra={"contract_id": contract_id})
            raise HTTPException(
    ...
    def create_contract(self, contract: CreateContract) -> ContractResponse:
        ...
        try:
            ...
            self.logger.info("Contract created successfully", extra={"data": created_contract.model_dump()})
    ...
    def update_contract(self, contract_id: int, contract: UpdateContract) -> ContractResponse:
        existing_contract = self.contract_repository.get_by_id(contract_id)
        
        if not existing_contract:
            self.logger.warning("Update failed: Contract not found", extra={"contract_id": contract_id})
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Contract not found"
            )
        ...
        payload = contract.model_dump()
        self.logger.info("Updating contract", extra={"contract_id": contract_id, "data": payload})

        try:
            updated_contract = self.contract_repository.update(contract_id, contract)
            self.logger.info("Contract updated successfully", extra={"data": updated_contract.model_dump()})
            return updated_contract
        except Exception:
            self.logger.exception("Failed to update Contract", extra={"contract_id": contract_id, "data": payload})
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="An unexpected error occurred while creating the contract"
            )
    ...
    def delete_contract(self, contract_id: int) -> None:
        ...
        if not existing_contract:
            self.logger.warning("Delete failed: Contract not found", extra={"contract_id": contract_id})
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Contract not found"
            )
        ...
        try:
            if not self.contract_repository.delete(contract_id):
                self.logger.error("Failed to delete Contract", extra={"contract_id": contract_id})
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="An unexpected error occurred while deleting the contract"
                )
            
            self.logger.info("Contract deleted successfully", extra={"contract_id": contract_id})
        except Exception:
            self.logger.exception("Failed to delete Contract", extra={"contract_id": contract_id})
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="An unexpected error occurred while deleting the contract"
            )
